...
Triangle::Triangle(const glm::dvec3 &a, const glm::dvec3 &b,
                   const glm::dvec3 &c, std::shared_ptr<Material> material)
...
GPUTriangle Triangle::toGPU(int materialIndex) const {
  GPUTriangle gpu{};
  gpu.a = glm::vec4(m_a.x, m_a.y, m_a.z, 0.0f);
  gpu.b = glm::vec4(m_b.x, m_b.y, m_b.z, 0.0f);
  gpu.c = glm::vec4(m_c.x, m_c.y, m_c.z, 0.0f);
